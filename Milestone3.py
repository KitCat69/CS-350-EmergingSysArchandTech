...
class CWMachine(StateMachine):
    ...
    ##
    ## endTransmission - flag used to determine whether
    ...
    def on_enter_dot(self):
        self.redLight.on(); sleep(0.5); self.redLight.off()  # Red light comes on for 500ms
    ...
    ## added by Cat 10/3/25
    def on_exit_dot(self):
        self.redLight.off()  # Red light forced off

    def on_enter_dash(self):
        self.blueLight.on()
        sleep(1.5)
        self.blueLight.off()  # Blue light comes on for 1500ms
        if DEBUG:
            print("* Changing state to blue - dash")

    def on_exit_dash(self):
        self.blueLight.off()  # Blue light forced off
            ...
            for word in wordList:
                ...
                for char in word:
                    ...
                    for x in morse:
                        ...
                        # Dot or dash? added by Cat 10/4/25
                        if x == ".":
                            self.doDot()
                            self.doDot()
                        elif x == "-":
...
greenButton.when_pressed = cwMachine.processButton  # added by Cat 10/4/25 whenPressed didn't work
